...
def test_add_allowed_tokens():
    if network.show_active() not in LOCAL_BLOCKCHAIN_ENVIRONMENTS:
        pytest.skip()
    account = get_account()
    savvy_finance_staking = deploy_savvy_finance_staking(account)
    tokens = get_tokens()
...
def test_token_is_allowed():
    ...
    add_allowed_tokens(savvy_finance_staking, tokens)
    remove_allowed_tokens(savvy_finance_staking, [tokens[0]])
    assert savvy_finance_staking.tokenIsAllowed(tokens[0]) == False
    assert savvy_finance_staking.tokenIsAllowed(tokens[1]) == True
    assert savvy_finance_staking.tokenIsAllowed(tokens[2]) == True
...
def test_stake_token():
    ...
    savvy_finance_staking.stakeToken(savvy_finance.address, stake_amount_1).wait(1)
    assert savvy_finance.balanceOf(savvy_finance_staking.address) == stake_amount_1
    assert (
        savvy_finance_staking.stakingData(savvy_finance.address, account.address)
        == stake_amount_1
    )
    assert savvy_finance_staking.stakers(0) == account.address
    assert savvy_finance_staking.stakersToUniqueTokensStaked(account.address) == 1
...
def test_stake_token_again():
    ...
    savvy_finance_staking.stakeToken(savvy_finance.address, stake_amount_2).wait(1)
...
def test_stake_token_another():
    ...
    savvy_finance_staking.stakeToken(savvy_finance_2.address, stake_amount).wait(1)
    assert savvy_finance_2.balanceOf(savvy_finance_staking.address) == stake_amount
    assert (
        savvy_finance_staking.stakingData(savvy_finance_2.address, account.address)
        == stake_amount
    )
    assert savvy_finance_staking.stakers(0) == account.address
    assert savvy_finance_staking.stakersToUniqueTokensStaked(account.address) == 2
...
def test_unstake_token():
    ...
    savvy_finance_staking.unstakeToken(savvy_finance.address, unstake_amount_1).wait(1)
...
def test_unstake_token_again():
    ...
    assert (
        savvy_finance_staking.stakingData(savvy_finance.address, account.address) == 0
    )
    with pytest.raises(exceptions.VirtualMachineError):
        savvy_finance_staking.stakers(0)
    assert savvy_finance_staking.stakersToUniqueTokensStaked(account.address) == 0
...
def test_unstake_token_another():
    ...
    savvy_finance_staking.unstakeToken(savvy_finance_2.address, stake_amount).wait(1)
    assert savvy_finance_2.balanceOf(savvy_finance_staking.address) == 0
    assert (
        savvy_finance_staking.stakingData(savvy_finance_2.address, account.address) == 0
    )
    assert savvy_finance_staking.stakers(0) == account.address
    assert savvy_finance_staking.stakersToUniqueTokensStaked(account.address) == 1
